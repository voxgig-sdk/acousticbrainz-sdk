# Acousticbrainz SDK test runner
...
class AcousticbrainzTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AcousticbrainzTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AcousticbrainzTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AcousticbrainzTestRunner.getenv("ACOUSTICBRAINZ_TEST_LIVE")
        override = AcousticbrainzTestRunner.getenv("ACOUSTICBRAINZ_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AcousticbrainzTestRunner.getenv(key)
        ...
        explain = AcousticbrainzTestRunner.getenv("ACOUSTICBRAINZ_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ACOUSTICBRAINZ_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AcousticbrainzTestRunner._test_control is not None:
            return AcousticbrainzTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AcousticbrainzTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AcousticbrainzTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AcousticbrainzTestRunner.load_test_control()
...
def load_env_local():
    AcousticbrainzTestRunner.load_env_local()


def env_override(m):
    return AcousticbrainzTestRunner.env_override(m)


def entity_data(v):
    return AcousticbrainzTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AcousticbrainzTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AcousticbrainzTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AcousticbrainzTestRunner.load_test_control()


def live_client_options():
    return AcousticbrainzTestRunner.live_client_options()


def live_delay_ms():
    return AcousticbrainzTestRunner.live_delay_ms()
