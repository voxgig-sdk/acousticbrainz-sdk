# Acousticbrainz TypeScript SDK



The TypeScript SDK for the Acousticbrainz API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const highlevel = await client.HighLevel().load({ id: 'example_id' })
...
| `tester(testopts?, sdkopts?)` | `AcousticbrainzSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `AcousticbrainzSDK.test(testopts?, sdkopts?)` | `AcousticbrainzSDK` | Create a test-mode client. |
...
| `client` | `client(): AcousticbrainzSDK` | Return the parent SDK client. |
...
acousticbrainz/
├── src/
│   ├── AcousticbrainzSDK.ts        # Main SDK class
...
await highlevel.load({ id: "example_id" })

// highlevel.data() now returns the loaded highlevel data
// highlevel.match() returns { id: "example_id" }
```
