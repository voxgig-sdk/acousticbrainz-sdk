...
//     (\`new AcousticbrainzSDK(...)\` / \`AcousticbrainzSDK.test(...)\` become
//     \`AcousticbrainzSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AcousticbrainzSDK'
...
const TEST_SEED = {"entity":{"high_level":{"test01":{"id":"test01"}},"low_level":{"test01":{"id":"test01"}},"metadata":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AcousticbrainzSDK(...)\` and any
// \`AcousticbrainzSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AcousticbrainzSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AcousticbrainzSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
