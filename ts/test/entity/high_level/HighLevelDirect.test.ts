...
describe('HighLevelDirect', async () => {
  ...
  test('direct-load-high_level', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-high_level', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ACOUSTICBRAINZ_TEST_HIGH_LEVEL_ENTID']
