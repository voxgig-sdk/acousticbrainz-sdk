...
describe('LowLevelDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AcousticbrainzSDK({
  ...
  test('direct-load-low_level', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-low_level', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ACOUSTICBRAINZ_TEST_LOW_LEVEL_ENTID']
