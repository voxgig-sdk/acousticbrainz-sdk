# Acousticbrainz Ruby SDK



The Ruby SDK for the Acousticbrainz API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.HighLevel` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/acousticbrainz-sdk/releases](https://github.com/voxgig-sdk/acousticbrainz-sdk/releases)
...
```ruby
client = AcousticbrainzSDK.test(testopts, sdkopts)
```
...
| `HighLevel` | `(data) -> HighLevelEntity` | Create a HighLevel entity instance. |
| `LowLevel` | `(data) -> LowLevelEntity` | Create a LowLevel entity instance. |
...
raise a `AcousticbrainzError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Acousticbrainz_sdk.rb       -- Main SDK module
...
The main module (`Acousticbrainz_sdk`) exports the SDK class
