...
class LowLevelEntity
  ...
  def make
    opts = @_entopts.dup
    LowLevelEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AcousticbrainzHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [LowLevel, Hash] the current LowLevel data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AcousticbrainzHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of LowLevel fields)
  def match_get
  ...
  #
  # @param reqmatch [LowLevelLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.LowLevel.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [LowLevel, Hash] the loaded LowLevel; raises AcousticbrainzError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AcousticbrainzHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
