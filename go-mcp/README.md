...
[MCP](https://modelcontextprotocol.io) server exposing the Acousticbrainz SDK as
two agent tools — `acousticbrainz_list` and `acousticbrainz_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/acousticbrainz-mcp)
make build

# 2. Provide credentials via the environment
export ACOUSTICBRAINZ_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user acousticbrainz \
  -- /absolute/path/to/acousticbrainz-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./acousticbrainz-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// acousticbrainz_list: first page of records
{ "entity": "high_level" }
{ "entity": "high_level", "query": { } }

// acousticbrainz_load: one record by id
{ "entity": "high_level", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/acousticbrainz-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user acousticbrainz \
     -- "$PWD"/dist/*/acousticbrainz-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `acousticbrainz_list` and `acousticbrainz_load` tools now appear
   in new sessions. Ask the agent to *"list high_level using acousticbrainz"*
   and it calls `acousticbrainz_list` with `{"entity":"high_level"}`.
...
```sh
export ACOUSTICBRAINZ_APIKEY=sk_live_xxx            # API key
export ACOUSTICBRAINZ_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `acousticbrainz_list` tool
...
### Call the `acousticbrainz_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `acousticbrainz_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `acousticbrainz_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ACOUSTICBRAINZ_APIKEY` | API key sent with every request. |
| `ACOUSTICBRAINZ_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

high_level | low_level | metadata

### Smoke test via HTTP (raw JSON-RPC)

```sh
./acousticbrainz-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"acousticbrainz_load","arguments":{"entity":"high_level","query":{"id":1}}}}'
```
