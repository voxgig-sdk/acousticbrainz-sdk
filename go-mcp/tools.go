...
type Args struct {
	Entity string         `json:"entity" jsonschema:"high_level | low_level | metadata"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AcousticbrainzSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "acousticbrainz_list",
		Description: "List records from Acousticbrainz. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "acousticbrainz_load",
		Description: "Load a single record from Acousticbrainz. " +
...
func runOp(client *sdk.AcousticbrainzSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AcousticbrainzSDK, name string) (sdk.AcousticbrainzEntity, error) {
	switch strings.ToLower(name) {
	case "high_level":
		return client.HighLevel(nil), nil
	case "low_level":
		return client.LowLevel(nil), nil
	case "metadata":
