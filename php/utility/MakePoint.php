...
// Acousticbrainz SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class AcousticbrainzMakePoint
{
    public static function call(AcousticbrainzContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof AcousticbrainzError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = AcousticbrainzHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = AcousticbrainzHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
