...
class LowLevelEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = AcousticbrainzSDK::test(null, null);
        $ent = $testsdk->LowLevel(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = low_level_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "low_level." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ACOUSTICBRAINZ_TEST_LOW_LEVEL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $low_level_ref01_data = null;
        if (count($low_level_ref01_data_raw) > 0) {
...
function low_level_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/low_level/LowLevelTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["low_level01", "low_level02", "low_level03"] as $k) {
    ...
    $entid_env_raw = getenv("ACOUSTICBRAINZ_TEST_LOW_LEVEL_ENTID");
