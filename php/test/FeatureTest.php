...
// Acousticbrainz SDK feature test
//
...
class FtCtrl extends AcousticbrainzControl
{
...
class FtClient
{
    ...
    private AcousticbrainzUtility $utility;

    public function __construct(AcousticbrainzUtility $utility, array $options)
    {
    ...
    public function get_utility(): AcousticbrainzUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AcousticbrainzContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AcousticbrainzContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AcousticbrainzContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AcousticbrainzConfig::make_feature($name);
    ...
    private static function buildUrl(AcousticbrainzSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AcousticbrainzError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AcousticbrainzResponse($fetched);
        }
    ...
    private function fail(AcousticbrainzContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AcousticbrainzContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
