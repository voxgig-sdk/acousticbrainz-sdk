...
The PHP SDK for the Acousticbrainz API. Provides an entity-oriented interface using PHP conventions.
...
$client = new AcousticbrainzSDK([
    "apikey" => getenv("ACOUSTICBRAINZ_APIKEY"),
]);
```
...
```php
[$result, $err] = $client->HighLevel(null)->load(["id" => "example_id"], null);
...
[$result, $err] = $client->Acousticbrainz(null)->load(
    ["id" => "test01"], null
);
