...
// Acousticbrainz SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AcousticbrainzTestFeature extends AcousticbrainzBaseFeature
{
    ...
    public function init(AcousticbrainzContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AcousticbrainzContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AcousticbrainzParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AcousticbrainzContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AcousticbrainzContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AcousticbrainzParam::call($ctx, $k);
