...
// Acousticbrainz SDK retry feature
...
class AcousticbrainzRetryFeature extends AcousticbrainzBaseFeature
{
    ...
    public function init(AcousticbrainzContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AcousticbrainzContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AcousticbrainzContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AcousticbrainzContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
