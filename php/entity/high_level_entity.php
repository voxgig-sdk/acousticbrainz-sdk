...
// Acousticbrainz SDK HighLevel entity
...
class HighLevelEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new HighLevelEntity($this->_client, $opts);
    }

    /**
     * @param HighLevel|array $args HighLevel data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AcousticbrainzHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return HighLevel|array The current HighLevel data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of HighLevel fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AcousticbrainzHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of HighLevel fields).
     */
     ...
     *
     * @param HighLevelLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed HighLevelLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return HighLevel|array The loaded HighLevel as an assoc-array at the
     *   SDK boundary; throws AcousticbrainzError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AcousticbrainzHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
