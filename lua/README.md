# Acousticbrainz Lua SDK

The Lua SDK for the Acousticbrainz API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:HighLevel(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Acousticbrainz(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── acousticbrainz_sdk.lua    -- Main SDK module
...
The main module (`acousticbrainz_sdk`) exports the SDK constructor
