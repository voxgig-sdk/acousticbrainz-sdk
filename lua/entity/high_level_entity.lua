...
local HighLevelEntity = {}
HighLevelEntity.__index = HighLevelEntity


function HighLevelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HighLevelEntity)
  self._name = "high_level"
  self._client = client
...
function HighLevelEntity:get_name()
  return self._name
end


function HighLevelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HighLevelEntity.new(self._client, opts)
end
...
function HighLevelEntity:mark_deleted()
  self._deleted = true
end


function HighLevelEntity:deleted()
  return true == self._deleted
end


function HighLevelEntity:data_set(args)
  if args ~= nil then
...
function HighLevelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HighLevelEntity:match_set(args)
  if args ~= nil then
...
function HighLevelEntity:match_get()
...
function HighLevelEntity:stream(action, args, callopts)
...
function HighLevelEntity:_run_op(ctx, post_done)
  local utility = self._utility
