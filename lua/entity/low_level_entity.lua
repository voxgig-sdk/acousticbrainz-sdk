...
local LowLevelEntity = {}
LowLevelEntity.__index = LowLevelEntity


function LowLevelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LowLevelEntity)
  self._name = "low_level"
  self._client = client
...
function LowLevelEntity:get_name()
  return self._name
end


function LowLevelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LowLevelEntity.new(self._client, opts)
end
...
function LowLevelEntity:mark_deleted()
  self._deleted = true
end


function LowLevelEntity:deleted()
  return true == self._deleted
end


function LowLevelEntity:data_set(args)
  if args ~= nil then
...
function LowLevelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LowLevelEntity:match_set(args)
  if args ~= nil then
...
function LowLevelEntity:match_get()
...
function LowLevelEntity:stream(action, args, callopts)
...
function LowLevelEntity:_run_op(ctx, post_done)
  local utility = self._utility
