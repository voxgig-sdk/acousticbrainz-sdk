...
Results: Successful response with high-level data.
...
Results: Successful response with low-level data.
...
Results: Successful response with submission count.
...
| --- | --- | --- | --- |
| HighLevel | `load` | `GET /{mbid}/high-level` | See reference |
| LowLevel | `load` | `GET /{mbid}/low-level` | See reference |
| Metadata | `load` | `GET /{mbid}/count` | See reference |

## Connect to the API

- Production server: `https://acousticbrainz.org/api/v1`
...
- `acousticbrainz_list`: List records for an entity. No active entity supports this operation.
- `acousticbrainz_load`: Load one record for an entity. Supported entities: `high_level`, `low_level`, `metadata`.
