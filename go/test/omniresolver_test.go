...
import (
	...
	omni "github.com/voxgig-sdk/acousticbrainz-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AcousticbrainzError); is {
				out := map[string]any{
					"name":    "AcousticbrainzError",
...
func omniCtx(arg any, client *sdk.AcousticbrainzSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AcousticbrainzSDK); is {
			client = live
