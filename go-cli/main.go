// Command Acousticbrainz-cli is a CLI + REPL for the Acousticbrainz SDK,
...
const prompt = "acousticbrainz"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "high_level low_level metadata"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ACOUSTICBRAINZ_APIKEY (api key), ACOUSTICBRAINZ_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ACOUSTICBRAINZ_APIKEY carries the API key and
	// ACOUSTICBRAINZ_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ACOUSTICBRAINZ_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ACOUSTICBRAINZ_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAcousticbrainzSDK(opts)
