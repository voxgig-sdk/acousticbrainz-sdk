...
boru-driven command-line client **and** interactive REPL for the Acousticbrainz
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/acousticbrainz-cli)
make build

# 2. See usage (words, entities, env vars)
./acousticbrainz-cli --help

# 3. Provide credentials once, via the environment
export ACOUSTICBRAINZ_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./acousticbrainz-cli load 1 high_level            # {id:1} shorthand
./acousticbrainz-cli load '{id:1}' high_level       # explicit match map

# 5. Override the API base URL for a single call
ACOUSTICBRAINZ_BASE=https://api.example.com ./acousticbrainz-cli load 1 high_level

# 6. No arguments -> interactive REPL
./acousticbrainz-cli
acousticbrainz> load 1 high_level
acousticbrainz> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/acousticbrainz-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/acousticbrainz-cli load 1 high_level
   ```
...
```sh
./acousticbrainz-cli load 1 high_level          # scalar shorthand for {id:1}
./acousticbrainz-cli load '{id:1}' high_level     # explicit match map
```
...
./acousticbrainz-cli load 1 high_level
```
...
Run with no arguments to open a REPL (prompt `acousticbrainz>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `high_level`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/acousticbrainz-cli`. |
...
(`./acousticbrainz-cli <expr>`) and interactively (the REPL), and expressions compose the
