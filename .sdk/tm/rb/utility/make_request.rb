...
module AcousticbrainzUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AcousticbrainzResponse.new({})
    ...
    if fetch_err
      ...
      response = AcousticbrainzResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AcousticbrainzResponse.new(fetched)
    else
